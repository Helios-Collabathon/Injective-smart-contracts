...
fn add_wallet(
    ...
    if info.sender.clone().to_string().eq(&wallet.clone().get_address()) {
        return Err(ContractError::CannotAddOwnAddressAsLinkedAddress);
    }

    let mut persona = PERSONAS.load(deps.storage, info.sender.clone()).unwrap_or(
        Persona::new(vec![])
    );
    ...
    let mut addresses = PERSONA_LOOKUP.load(
        deps.storage,
        wallet.clone().get_id()
    ).unwrap_or_default();
