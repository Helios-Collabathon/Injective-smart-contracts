use crate::{ msg::QueryMsg, persona::Persona, state::{ PERSONAS, PERSONA_LOOKUP }, wallet::Wallet };
...
fn get_persona(deps: Deps, address: Addr) -> StdResult<Persona> {
    Ok(PERSONAS.load(deps.storage, address).unwrap_or(Persona::new(vec![])))
}

fn get_persona_by_linked_wallet(deps: Deps, wallet: Wallet) -> StdResult<Vec<Persona>> {
    let addresses = PERSONA_LOOKUP.load(deps.storage, wallet.get_id()).unwrap_or_default();
